...
{
	//changed this to select the search URI based on what tab is currently visible. 
	//this is because vendors may wish to specify their own search URL for certain tabs	
	//If this breaks anything let me know, as I'm not sure whether it will play nice with the hippogrid stuff. JM
	...
	if(url.empty()) //let's just be extra certain, fall back on old method if necessary
	{
		...
		panelProvidesURL = true;
	}
	
	if (search_text.empty() && !panelProvidesURL) {
